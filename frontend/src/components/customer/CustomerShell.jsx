...
function CustomerShell({ customerName, children }) {
  ...
  const onSearchPage = location.pathname === "/customer/search";
  ...
  const handleSearchSubmit = (e) => {
    e.preventDefault();
    const q = search.trim();
    navigate(q ? `/customer/search?q=${encodeURIComponent(q)}` : "/customer/search");
  };
  ...
  return (
    <div className="cd-shell">
      <header className="cd-topbar">
        ...
        <form className="cd-topbar-search" onSubmit={handleSearchSubmit}>
          <span className="material-symbols-outlined">search</span>
          <input
            type="search"
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search food across all restaurants…"
        ...
        <aside className="cd-sidebar">
          ...
          <div className="cd-sidebar-footer">
            <div className="cd-sidebar-tip">
              <p className="cd-sidebar-tip-title">Search anything</p>
              <p className="cd-sidebar-tip-text">
                Use the search bar to find dishes from any restaurant — no need to pick a store
                first.
              </p>
            </div>
          </div>
